 ...
 *
 */
class SCH_Form_Family extends CRM_Core_Form
{
    const
    ...
    function preProcess( ) {
        ...
        if ( $this->_studentId ) {
            ...
            $subType = SCH_Utils_Query::getSubType( $this->_parentId );
            if ( $subType != 'Parent' &&
                 $subType != 'Staff' ) {
            ...
            CRM_Utils_System::setTitle( ts( 'SCH Family Information for %1',  array( 1 => $displayName ) ) );
        }

        // set up tabs
        require_once 'SCH/Form/Family/TabHeader.php';
        SCH_Form_Family_TabHeader::build( $this );
    }
...
";
        ...
        $title = SCH_Form_Family_TabHeader::getSubPageInfo( $this, $className );
        ...
        if ( $this->controller->getButtonName('submit') == "_qf_{$className}_next" ) {
            ...
            if ( $taskList['is_completed'] ) {
                ...
                $session->setStatus( ts( "Thank you for completing the SCH Family Information Online Forms.
